...
- Outbound delivery is FIFO per connector/local user. Only unsent claim leases expire automatically; persisted delivery attempts fence uncertain remote mutations across restart, timeout and local user deletion. Acknowledgment and event/system status commit together. Generic and suite synchronous writes require completion responses; 202, 408, server errors and lost mutation responses remain blocked. Suite sync exposes bounded attempt listing and read-back diagnostics, which never automatically clear a fence. Recovery requires operation-bound step-up, the exact expired attempt, operator-confirmed worker/remote quiescence and an atomic audit. Clearing an uncertain-create guard additionally requires explicit no-create confirmation and empty externalId read-back. Upgrade requires stopping old workers and settling their remote operations. PR08b still owns assignment-aware scope, desired-state supersession, revisions and groups. `internal/store/sync_delivery_test.go`, `internal/sync/delivery_test.go` and the recovery API tests cover stale claims, restart, deletion races, real delayed HTTP responses, audit rollback and recovery gates.
