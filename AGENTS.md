...
- OIDC `prompt=login` and `max_age=0` require one fresh login per authorization interaction; positive ages persist an absolute proof deadline checked atomically at token registration. `prompt=none` never creates UI. Supported requested assurance is password or ordinary MFA; recovery cannot satisfy MFA. Validated requests are stored in bounded five-minute interactions, bound to an HttpOnly browser cookie, the original account/session when present, and the exact resulting login session. Session creation and interaction satisfaction commit together; code creation consumes the interaction in its transaction. Cancellation and completion serialize, with the first committed operation winning. Per-app policy settings and revisions remain PR05b.
