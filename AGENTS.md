...
- OIDC `prompt=login` and `max_age=0` require one fresh login per authorization interaction; positive ages persist an absolute proof deadline checked atomically at token registration. `prompt=none` never creates UI. Supported requested assurance is password or ordinary MFA; recovery cannot satisfy MFA. Validated requests are stored in bounded five-minute interactions, bound to a signed HttpOnly browser cookie, the original account/session when present, and the exact resulting login session. Session creation and interaction satisfaction commit together; code creation consumes the interaction in its transaction. Cancellation and code issuance serialize, with the first committed operation winning. A completed password/MFA proof still creates an unbound active-user session if its interaction expired or was cancelled, with an explicit authorization-restart signal; it cannot resume that request. Capacity pressure evicts only anonymous unfinished interactions. Authorize throttles by signed browser identity (IP fallback) and returns protocol errors only after validating the callback. Per-app policy settings and revisions remain PR05b.
