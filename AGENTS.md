...
- Step-up requires password plus an enrolled TOTP, push, or passkey when any factor is enrolled. Grants and pending challenges bind to user, session, and exact method/path (enrollment preparation maps to its final mutation). Recovery grants only authorize replacement TOTP/passkey enrollment. Completion and audit commit atomically; cancellation burns pending challenges and raced grants. Step-up does not rewrite login authentication evidence.
