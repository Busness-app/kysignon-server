...
func TestSyncDeliveryRecoveryRequiresStepUpAndConfirmation(t *testing.T) {
	...
	if err := db.ConfigureSystem(sys.ID, "suite_webhook", "scim", "sensitive-credential-fixture", nil); err != nil {
		t.Fatal(err)
	}
	rr = adminRequest(t, srv, "POST", base+"/"+events[0].ClaimToken+"/read-back", cookie, "")
	if rr.Code != 502 {
	...
	if err := db.ConfigureSystem(sys.ID, "scim", "suite_webhook", "", nil); err != nil {
		t.Fatal(err)
	}
