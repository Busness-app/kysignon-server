...
export function AdminAppAccess({ app, onClose, onChanged }: { app: AppRecord; onClose: () => void; onChanged: () => void }) {
 ...
 return <div className="admin-page">
  ...
  {app.systemId&&<p className="text-muted">Provisioning continues with its existing scope. These controls do not remove or disable downstream accounts.</p>}
  ...
  <section className="settings-section"><div className="section-header"><h2>Access policy</h2></div><div className="modal-body">
   ...
   <p role="status">{users.page?`${users.page.losingAccess} users would lose access with this policy. Preview reflects current directory membership.`:'Loading access preview...'}</p>
